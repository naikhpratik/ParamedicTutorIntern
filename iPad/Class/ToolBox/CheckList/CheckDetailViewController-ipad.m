//
//  CheckDetailViewController-ipad.m
//  Quiz
//
...
@interface CheckDetailViewController_ipad ()

@end

@implementation CheckDetailViewController_ipad
...
- (void)viewDidLoad
{
	...
	_lbTitle.text = chapterBean.name;
...
-(IBAction)clearAll: (id)sender {
    if ([_arr_checks count] != 0) {
        [_arr_checks removeAllObjects];
    }
    [[SqlMB getInstance] clearCheckedFromDBForToolBoxWithCharpterID:chapterBean.chapterID];
    [[SqlMB getInstance] getCheckListFromDBForToolBox:_arr_checks ChapterId:chapterBean.chapterID];
    NSLog(@"chapterBean.chapterID = %d",chapterBean.chapterID);
